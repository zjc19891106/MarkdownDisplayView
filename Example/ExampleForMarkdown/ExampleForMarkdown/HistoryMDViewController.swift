...
final class HistoryMDViewController: UIViewController {
    ...
    private func prepareMessages() {
        let baseTableArray = [
            ...
            | 列一标题比较长一些用来折行 | 列二 | 列三请横向滑到这里看底部是否被裁切 |
            | --- | --- | --- |
            | 这是一个很长的表格单元格内容，用来触发多行折行，看看最后一行是否会被裁切掉一部分文字。 | 短 | 第三列短内容 |
            | 这是一个很长的表格单元格内容，用来触发多行折行，看看最后一行是否会被裁切掉一部分文字。这是一个很长的表格单元格内容，用来触发多行折行，看看最后一行是否会被裁切掉一部分文字。 | 这是一个很长的表格单元格内容，用来触发多行折行，看看最后一行是否会被裁切掉一部分文字。 | 横向滚到这一列后，最后一行「裁切检查完成」这几个字必须完整露出，不能被底边切掉。裁切检查完成 |
            | **加粗**、[链接](https://example.com/path) 和 `code` 混排，并且后面继续跟一段足够长的中文，确保折到最后一行时标点与字脚都还能看见。 | 质能方程 $E=mc^2$ 后面再接一段很长的说明文字，用来验证单元格里行内公式和普通文字一起折行时底部不会被裁掉。 | 第三列继续加长：滚动复用单元格时内边距不能丢，最后一行「第三列结束」也要完整显示。第三列结束 |
            """,
